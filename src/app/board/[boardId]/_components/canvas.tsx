...
export default function Canvas({ boardId }: CanvasProps) {
  ...
  const insertLayer = useMutation(
    ({ storage, setMyPresence }, LayerType: LayerType.Ellipse | LayerType.Note | LayerType.Rectangle | LayerType.Text, position: Point) => {
  ...
  const updateSelectionNet = useMutation(
    ({ storage, setMyPresence }, current: Point, origin: Point) => {
      if (layerIds) {
        ...
        setCanvasState({
          mode: CanvasMode.SelectionNet, origin, current,
        });
    ...
    setMyPresence({
      ...
      pencilDraft:
        pencilDraft.length === 1 &&
          ...
          pencilDraft[0][1] === point.y ? pencilDraft
  ...
  const resizeSelectedLayer = useMutation((
    ...
    const bounds = resizeBounds(
      canvasState.initialBounds, canvasState.corner,
      point
    )
  ...
  }, [continueDrawing, startMultiSelection, updateSelectionNet, camera, canvasState, resizeSelectedLayer, translateSelectedLayers])
  ...
  return (
    <main className="h-screen w-full relative bg-neutral-100 touch-none">
      ...
      <Toolbar canvasState={canvasState} setCanvasState={setCanvasState}
        canRedo={canRedo} canUndo={canUndo} redo={history.redo} undo={history.undo} />
      <SelectionTools camera={camera} setLastUsedColor={setLastUsedColor} />
      <svg className="h-[100vh] w-[100vw]" onWheel={onWheel}
